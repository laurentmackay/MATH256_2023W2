...
\vfill
\student{
Restricting ourselves to 2 cartesian dimensions, we get\[ u_{xx} + u_{yy} = 0\]
\vfill
...
\enum{
\item Consider a region in the $x,y$-plane, the domain for the problem. 
\item Specify the value of some unknown function $u(x,t)$ along the boundaries of the region. 
\item Solve Laplace's equation: \[ u_{xx} + u_{yy} = 0\] 
\item Impose the boundary conditions.}
\vfill
\raggedleft{\includegraphics[width=7.2cm]{images/dirichlet_domain.pdf}}
\vfill
}

\slide[A Simple Dirichlet Problem]{
...
\twomini[.4]{.5}{.5}{\student{\algn{\Delta u &=0 \\
u(0,y)&=0 &\text{for } 0<y<h\\
u(x,h)&=0 &\text{for } 0<x<w\\
u(w,y)&=0 &\text{for } 0<y<h\\
u(x,0)&=f(x) &\text{for } 0<x<w }}}{\vspace{.5cm} \centerline{\includegraphics[width=.85\columnwidth]{images/dirichsetup.pdf}}}
...
\twomini[.15]{.5}{.5}{\uline{Horizontal Problem:}\[X\pp+\lambda X =0 \]}{\uline{Vertical Problem:}\[Y\pp-\lambda Y =0 \]}

}
}

\slide[Solving Dirichlet Problems]{
\twomini[.35]{.5}{.45}{\[X\pp+\lambda X =0 \]}{ \centerline{\includegraphics[width=.85\columnwidth]{images/dirichsetup.pdf}} }
\vfill
What are the boundary conditions? What are the allowed solution and values of $\lambda$?
\vfill
\student{
\uline{Horizontal Problem:}\vfill
BCs: $X(0)=X(w)=0$
...
\slide[Solving Dirichlet Problems]{
\twomini[.37]{.5}{.45}{\[Y\pp-\lambda Y =0 \] \vfill\[ \lambda = \left(\frac{n\pi}{w}\right)^2\] \vfill }{ \centerline{\includegraphics[width=.85\columnwidth]{images/dirichsetup.pdf}} }
\vfill
What are the boundary conditions? What types of solutions do we obtain?
\vfill
\student{
\vfill
\uline{Vertical Problem:} BCs: $Y(h)=0, Y(0)=f(x)$.\vfill
\algn{\text{Try }Y_n=e^{ry}:\qquad r^2e^{ry} - \left(\frac{n\pi}{w}\right)^2 e^{ry}&=0 \\
r^2 = \left(\frac{n\pi}{w}\right)^2&=0& \Rightarrow r&=\pm\frac{n\pi}{w} }\vfill
\[Y_n(y) = Ae^{\frac{n\pi}{w} y} + B e^{-\frac{n\pi}{w} y}\]
}
}
...
u(x,0)&=f(x) = \sum_{n=1}^{\infty} a_n \sin \left( \frac{n\pi}{w} x \right)  \sinh \left( n\pi \frac{h}{w} \right)  \intertext{need equality between the two series}
\Rightarrow a_n &= \frac{b_n}{ \sinh \left( n\pi \frac{h}{w} \right) } 
}
}
}
...
\vfill
 \centerline{\includegraphics[width=.5\columnwidth]{images/dirichsetup2.pdf}}
\vfill
...
\vfill
\student{
\[\uline{\text{Horizontal Prob:}}\quad X\pp+\lambda X =0 \qquad \uline{\text{Vertical Prob:}} Y\pp-\lambda Y =0 \]
...
&=-2\sum_{n=1}^{\infty}  \frac{(-1)^n}{n\pi} \frac{\sin \left( n\pi x \right)}{\sinh(n\pi)} \sinh \left( n\pi (1-y)\right) \\& \phantom{=}+\frac{\sinh(3\pi x)}{\sinh(3\pi)}\sin(3\pi y)}
\vfill
