...
b_n&=\frac2T\int_0^T f(t) \sin\paren{\frac{2n\pi t}{T}}dt && \student{=\frac2T\int_\alpha^{\alpha+T} f(t) \sin\paren{\frac{2n\pi t}{T}}dt}}
\vfill
\student{We always integrate over one period, and can choose $\alpha$ to make the integrals simpler to evaluate.}
}
...
\uline{cosine terms:}
\algn{
 a_n &=  \frac22 \int_{-1}^1\underbrace{ \underbrace{x}_{\text{odd func.}} \underbrace{\cos(n\pi x)}_{\text{even func.}}}_{\text{odd func.}}dx \\
 &=0 \qquad \Rightarrow  \quad  \text{No $\cos$ terms in the Fourier Series} }
...
\uline{sine terms:}
\algn{
b_n &= \frac11 \int_{-1}^1 x \sin(n\pi x) dx  \quad  \text{let} \quad  \arr{ll}{ u=x& du=dx \\ dv=\sin(n\pi x)dx & v=-\frac{\cos(n\pi x)}{n\pi} }\\
...
\[f(x) = -2 \sum_{n=1}^\infty \frac{(-1)^n}{n\pi} \sin(n\pi x) \]
}
}
...
\algn{b_n &= \frac12 \int_{-2}^2 \underbrace{\underbrace{ |x|}_{\text{even func.}} \underbrace{\sin\left(  n \frac{\pi}{2} x \right)}_{\text{odd func.}}}_{\text{odd func.}} dx\\ \Rightarrow  b_n&=0
}
...
Even Function: $\qquad f(x) \approx \frac{a_0}{2}+\sum_{n=1}^\infty a_n \cos(\omega_n x) $\hfill$ \omega_n=n\frac{2 \pi}{T}$
\vfill
\student{Proof: \[ b_n = \int_{-L}^L \underbrace{\text{even func.} \times \sin(\omega_n)}_{\text{odd func.}} = 0 \]}\hfill$ \omega_n=n\frac{2 \pi}{T}$\vfill
Odd Function: $\qquad f(x) \approx \sum_{n=1}^\infty b_n \sin(\omega_n x) $\hfill$ \omega_n=n\frac{2 \pi}{T}$\vfill
...
\slide[General Fourier Series of a $T$-periodic Function]{
Even Function: \hfill$f(x) \approx \frac{a_0}{2}+\sum_{n=1}^\infty a_n \cos(\omega_n x) $\hfill~
\vfill
\centerline{\student{Fourier Cosine Series}}\vfill
Odd Function: \hfill$f(x) \approx \sum_{n=1}^\infty b_n \sin(\omega_n x) $\hfill~\vfill
\centerline{\student{Fourier Sine Series}}\vfill

Neither even or odd: \hfill $ f(x) \approx \frac{a_0}{2}+\sum_{n=1}^\infty a_n \cos(\omega_n x) + b_n \sin(\omega_n x) $ \hfill ~ \vfill
\centerline{\student{Fourier Series}}
\vfill
In all cases: $ \omega_n=n\frac{2 \pi}{T}$
}


\slide[Observations:]{

\itmz{\item Take more terms in the Fourier Series and it appears to converge to $f(x)$ , (even if $f(x)$ has discontinuities!) \vfill
\item The coefficients $a_n$ \& $b_n$ that we calculate decrease to zero as $n\to\infty$\vfill
\item The ``DC'' component $\frac{a_0}{2}$ is simply the mean value of $f(x)$\vfill
\item Fourier Series overshoot/undershoot the function $f(x)$ at points of discontinuity.\vfill
\item The above are common features of Fourier series expansions with arbitrary functions.\vfill
}
}
