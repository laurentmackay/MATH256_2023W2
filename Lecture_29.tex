...
\slide[D'Alembert's Method: $y_{tt}=c^2y+xx$]{
...
\vfill
\student{
Let $F$ be the odd periodic extension of $f(x)$, and $G$ be the odd periodic extension of $g(x)$. Then we have\vfill
\[A(z) = \frac12 \left[ F(z) - \frac1c\int_0^z G(x)dx\right] \quad  \text{and} \quad B(z) = \frac12 \left[ F(z) + \frac1c\int_0^z G(x)dx \right]\]
\vfill
Proof: DiffyQs \S 3.8.2
}
}
...
We have odd  extensions of the initial condition moving left and right.\vfill This leads to reflection of the wave at the domain boundaries.
}
}
